        ...
        public class DatabaseManager
        {
            ...
            * This method is private, meaning it is intended to be used only within the DatabaseManager class. */
            private static User RetrieveUserInformation(string username, string password)
            {
                ...
                using (SqlConnection sqlCon = new SqlConnection(@"Data Source=LAB109PC08\SQLEXPRESS; Initial Catalog=Users; Integrated Security=True;"))
                {
                    sqlCon.Open(); // Open SQL connection

                    string Query = "SELECT COUNT(1) FROM UserData WHERE Username=@Username AND Password=@Password";
                    string imgQuery = "SELECT img FROM UserData WHERE Username=@Username AND Password=@Password";
